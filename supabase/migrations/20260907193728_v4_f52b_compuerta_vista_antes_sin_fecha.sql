-- Indice para el lookup por URL canonica. El unico que habia era
-- (fecha, url_canonica), que no sirve para buscar por url sola: fecha es la
-- columna lider. Este va al reves y resuelve el min(fecha) de una.
create index if not exists candidatas_raw_url_fecha_idx
  on public.candidatas_raw (url_canonica, fecha);
...
returns table (
  candidata_id uuid, url text, url_canonica text, titulo text,
  dominio_norm text, fecha_pub timestamptz, es_prioritaria boolean,
  descartada_por text, motivo text, regla_id uuid, valor_que_matcheo text
)
...
ev as (
  select s.*,
    ...
    (s.fecha_confiable and s.fecha_pub < (select cfg.corte from cfg)
    ...
    -- La primera vez que vimos esta URL, si fue antes de hoy. Para las notas sin
    -- fecha propia es la unica estimacion de frescura que tenemos, y NO inventa
    -- nada: no dice "se publico tal dia" sino "la venimos viendo desde tal dia".
    (select min(c2.fecha) from candidatas_raw c2
      where c2.url_canonica = s.url_canonica and c2.fecha < p_fecha) as primera_vez
  from suscritas s
),
gated as (
  select e.*,
    ...
    case
      ...
      -- Sin fecha propia + ya estaba en el pool hace dias = mueble de la home,
      -- no noticia. Es la unica compuerta que puede frenar lo evergreen de las
      -- fuentes html, donde el 100% de las notas viene sin fecha.
...
select v.id, v.url, v.url_canonica, v.titulo, v.dominio_norm, v.fecha_pub, v.es_prioritaria,
  ...
  case v.veredicto
    ...
    when 'antiguedad'         then 'publicada antes del dia del clipping (ventana de '||(select cfg.ventana_h from cfg)||' h)'
    when 'fecha_futura'       then 'la fecha de publicacion cae despues del dia del clipping'
    when 'vista_antes_sin_fecha' then 'no trae fecha propia y ya estaba en el pool desde el '||v.primera_vez::text
