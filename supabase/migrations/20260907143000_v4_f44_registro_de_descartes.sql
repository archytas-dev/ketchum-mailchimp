...
--
-- Estado previo: el bloque de registro dentro de normalizar_y_compuertas() nunca
-- pudo correr. Insertaba sin `fase`, que es NOT NULL y sin default, asi que toda
-- llamada con p_registrar=true —que es el default— terminaba en 23502. Por eso
-- notas_descartadas tenia 0 filas con etapa='compuerta': no es que faltara
-- correrla, es que no podia.
--
-- Y aun arreglando eso, solo contemplaba los descartes por regla. La antiguedad,
-- el titulo pobre y la deduplicacion —que son el 95% del embudo medido— no
-- dejaban rastro, que es justo lo que la pantalla "Notas que no entraron" de la
-- Fase 7 viene a leer.
--
-- La evaluacion pasa a v4_evaluar_candidatas(): se define una sola vez y la usan
-- tanto el registro como el retorno, asi no pueden divergir.

-- ---------------------------------------------------------------------------
-- Idempotencia (mandamiento 6): la tabla solo tenia PK sobre `id`, un uuid
-- generado, asi que el `on conflict do nothing` anterior no matcheaba nada y
-- re-ejecutar el mismo dia habria duplicado todo el embudo.
-- Parcial sobre etapa='compuerta' para no tocar las filas que ya escribio la v3.
-- ---------------------------------------------------------------------------
create unique index if not exists notas_descartadas_compuerta_uniq
  on public.notas_descartadas (client_id, fecha, md5(coalesce(url,'')), motivo)
  where etapa = 'compuerta';

-- ---------------------------------------------------------------------------
-- El evaluador: del pool compartido al veredicto por nota, con el porque.
-- Orden (decision del design doc): suscripcion -> fecha -> compuertas -> dedup.
-- La dedup va ultima a proposito: si una nota se cae por regla, ese descarte
-- dice mas que "era repetida".
-- ---------------------------------------------------------------------------
...
ev as (
  select s.*,
    ...
    -- 2. Marca del cliente: le gana al descarte por tema.
    exists (select 1 from reglas r
             where r.compuerta = 'entra_si_o_si' and r.tipo = 'patron_titulo'
               and (coalesce(s.titulo,'') || ' ' || coalesce(s.snippet,'')) ~* r.valor
           ) as menciona_marca,
    -- 3. Descarte duro por tema, con las excepciones que no se expresan en regex.
    (select r.id from reglas r
    ...
    (s.fecha_confiable
     and s.fecha_pub < now() - make_interval(hours => (select cfg.ventana_h from cfg))) as es_vieja,
...
gated as (
  select e.*,
    ...
    case
      ...
      when e.menciona_marca            then null   -- la marca gana sobre 3 y 4
...
select
  ...
  case v.veredicto
    ...
    when 'antiguedad'         then 'publicada fuera de la ventana de '
  ...
  case
    ...
    -- El valor que disparo la regla, no el dominio por defecto: se busca primero
    -- en el texto y despues en la URL, segun donde matchee de verdad.
    else coalesce(
...
comment on function public.v4_evaluar_candidatas(uuid, date) is
  'Evalua el pool del dia para un cliente y devuelve, por nota, si entra y por que no. Una sola definicion para los cuatro clientes; la usan normalizar_y_compuertas() y el dashboard.';

-- ---------------------------------------------------------------------------
-- La funcion publica: misma firma que antes, ahora apoyada en el evaluador.
-- ---------------------------------------------------------------------------
...
begin
  if p_registrar then
    ...
    select
      ...
      -- Recuperable = el equipo podria subirla a mano desde el dashboard.
      -- Una repetida o una ya enviada no lo son: subirlas seria el bug.
      e.descartada_por in ('regla_tema', 'titulo_pobre', 'antiguedad')
