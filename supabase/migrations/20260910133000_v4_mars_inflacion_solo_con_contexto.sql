-- Copia la excepción explícita de la v3: "Inflación" sola no habilita una
-- nota de Mars. Si aparece junto con una keyword propia del cliente, sí.
...
with cliente as (
  ...
  select c.*,
    ...
    exists (
      select 1 from keywords k
      where (' ' || c.titulo_norm || ' ') like ('% ' || k.keyword_norm || ' %')
        and not ((select slug from cliente) = 'mars' and k.keyword_norm = 'inflacion')
...
comment on function public.v4_candidatas_aceptadas_operativo(uuid, date) is
  'Mars replica la v3: Inflación sola no alcanza; necesita otra keyword de Mars o marca directa.';
