...
export async function generateResumen(
  ...
  const sys = `Sos analista de PR para ${cfg.term}. Recibís las notas de hoy separadas en EXCLUSIVAS y ${cfg.coLabel} (título + snippet). REGLA OBLIGATORIA: para CADA eje que reciba al menos UNA nota, DEBÉS devolver un párrafo NO vacío (2 a 5 oraciones) que sintetice esas notas, aunque sea una sola nota o te parezcan menores — resumí lo que haya, agrupando las del mismo hecho (no las repitas). TODAS las notas recibidas deben quedar reflejadas en el párrafo de su eje, incluso si no tienen relación directa con ${cfg.term} ni con el sector: la selección ya la hizo una persona, no la cuestiones ni descartes ninguna por relevancia. No enumeres, no repitas los títulos textualmente, no saludes, no uses la palabra "hoy". Devolvé UN objeto JSON {"exclusivas":"<parrafo o cadena vacia>","competencia":"<parrafo o cadena vacia>"} (la clave "competencia" corresponde a ${cfg.coLabel}). Devolvé cadena vacía SOLO si el eje no recibió NINGUNA nota; si recibió aunque sea una, el párrafo NO puede ir vacío.`;
